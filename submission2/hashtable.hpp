...
bool HashTable<T>::contains(const T &x) const {
  const auto &indexList = hashTable[myhash(x)];

  return find(begin(indexList), end(indexList), x) != end(indexList);
}
...
void HashTable<T>::rehash() {
    auto old = hashTable;
    size_t newSize = prime_below(2 * hashTable.size());
    hashTable.resize(newSize);
    makeEmpty();

    theSize = 0;

    for (const auto &indexList : old) {
